...
//#include "CS8ServerV1.nsmap"
//#include "CS8ServerV2.nsmap"
//#include "CS8ServerV3.nsmap"
#include <math.h>
using std::string;

//---------------------------------------Server V0---------------------------------//
bool TX60::Login(const string& url, const string& userName, const string& password){
	if(mLogin)
		delete mLogin;
	if(mLoginResponse)
...
bool TX60::SetJoints(const std::vector<double>& joints){
	if(joints.size() != 6)
		return false;
	if(!mIsLoggedIn)
		return false;
...
bool TX60::GetRobotJoints(std::vector<double>& joints){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::GetRobotCartesianPosition(std::vector<double>& position){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::GetRobots(std::vector<int>& robots){
	if(!mIsLoggedIn)
		return false;
...
//----------------------------------------Server V1----------------------------------------//
bool TX60::GetApplications(std::vector<std::string>& appNames){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::GetJointRange(std::vector<double>& min, std::vector<double>& max){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::MoveJoints(std::vector<double> jnts){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::InverseKinematics(std::vector<double> pos, std::vector<double> start, std::vector<double> &jnts){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::ForwardKinematics(std::vector<double> jnts, std::vector<double> &pos){
	if(!mIsLoggedIn)
		return false;
...
bool TX60::MoveLine(std::vector<double> pos){
	if(!mIsLoggedIn)
		return false;
...
void TX60::GetRxRyRzCoord(ns6__Frame *x_fr, double *x_Rx, double *x_Ry, double *x_Rz)
{
...
void TX60::SetFrameFromRxRyRz(ns6__Frame *x_fr, double x_Rx, double x_Ry, double x_Rz)
{
...
void TX60::InitializeMotionDesc(ns6__MotionDesc* md){
...
void TX60::InitializeConfig(ns6__Config * conf){
